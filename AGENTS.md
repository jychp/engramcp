...
Agent → correct_memory → [WM-first mutations + graph-aware merge + audit]
```
...
|---|---|---|
| 7 | `server.py` | MCP interface + consolidation/retrieval assembly/wiring + correction audit flows (WM-first `contest`/`annotate`/`split_entity`/`reclassify`, graph-aware `merge_entities`) |
