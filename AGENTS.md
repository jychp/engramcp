...
- **Tier 2 semi-real baseline**: `tests/scenarios/test_tier2_corporate_timeline.py` codifies archived S9-style timeline evolution checks (position changes vs. false contradiction inflation)
...
- **Ground-truth verification**: use `make verify-scenario-ground-truth` to run Tier 2 scenario evals + verification, or `make verify-scenario-ground-truth-only` to validate against existing tier2 metrics. Verification covers Tier 2 scenario metric coverage (`tests/scenarios/fixtures/ground_truth_tier2.json`) and Tier 3 subset structure plus explicit negative samples (`tests/scenarios/fixtures/ground_truth_tier3_flight_logs_subset.json`)
- **Ground-truth CI gate**: `checks.yml` includes a dedicated `python-ground-truth` job that runs Tier 2 scenario evals + verifier and uploads `reports/ground-truth-verification.json`
