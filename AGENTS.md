...
- **Perf command targets**: use `make test-retrieval-perf` for the deep/branching retrieval latency guardrail, and `make test-bounded-resources` for sustained-load CPU/memory bounded-resource guardrails
...
- **Bounded-resource guardrail**: `tests/integration/test_bounded_resource_usage.py` enforces sustained-load bounded-resource behavior (working-set cap + CPU/wall-time per retrieval + memory-growth budget)
