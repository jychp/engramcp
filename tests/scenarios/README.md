...
- `make test-scenarios-tier3` (runs only Tier 3 non-`real_llm`, emits `reports/pytest-scenarios-tier3.xml`)
...
- `make verify-scenario-ground-truth` (runs Tier 2 + Tier 3 CI-safe scenarios, combines metrics, then verifies Tier 2 + Tier 3 expectations; emits `reports/ground-truth-verification.json`)
- `make verify-scenario-ground-truth-only` (runs only verifier against existing metrics file; optional `METRICS_PATH=...`)
...
- `tests/scenarios/fixtures/ground_truth_tier3_flight_logs_subset.json` (Tier 3 subset schema/alias baseline + runtime metric expectations + explicit negative validation samples)
- `tests/scenarios/helpers/ground_truth_verify.py` (verification CLI)

Tier 3 scenario modules currently include:
- `tests/scenarios/test_tier3_flight_logs_regression.py` (semi-real flight-log subset ingestion/retrieval with extraction proxy, merge-precision proxy, and retrieval usefulness metrics)
